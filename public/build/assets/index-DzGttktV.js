var U=Object.defineProperty;var P=(d,e,t)=>e in d?U(d,e,{enumerable:!0,configurable:!0,writable:!0,value:t}):d[e]=t;var r=(d,e,t)=>P(d,typeof e!="symbol"?e+"":e,t);import{C as c,E as p}from"./app-uEaRFPm-.js";import{g as L,e as i,G as y,s as A,a as x}from"./GameStateService-SGESB-EJ.js";class w{static info(e){console.log(`[INFO] [${new Date().toISOString()}] ${e}`)}static warn(e){console.warn(`[WARN] [${new Date().toISOString()}] ${e}`)}static error(e,t){console.error(`[ERROR] [${new Date().toISOString()}] ${e}`,t)}static event(e,t){}}class k{constructor(){r(this,"keys",{up:!1,down:!1,left:!1,right:!1})}init(){console.log("[SYSTEM] InputSystem - Continuous Sensors ONLINE."),window.addEventListener("keydown",e=>this.handleKeyDown(e)),window.addEventListener("keyup",e=>this.handleKeyUp(e))}handleKeyDown(e){this.updateKeyState(e.code,!0),c.emit({type:p.INPUT_KEY_DOWN,timestamp:Date.now(),data:{code:e.code}})}handleKeyUp(e){this.updateKeyState(e.code,!1),c.emit({type:p.INPUT_KEY_UP,timestamp:Date.now(),data:{code:e.code}})}updateKeyState(e,t){switch(e){case"KeyW":case"ArrowUp":this.keys.up=t;break;case"KeyS":case"ArrowDown":this.keys.down=t;break;case"KeyA":case"ArrowLeft":this.keys.left=t;break;case"KeyD":case"ArrowRight":this.keys.right=t;break}}preUpdate(e){}update(e){c.emit({type:"PLAYER:INPUT_STATE",timestamp:Date.now(),data:{...this.keys}})}postUpdate(e){}destroy(){console.log("[SYSTEM] InputSystem - Continuous Sensors OFFLINE.")}}const B=new k;class Y{constructor(){r(this,"currentMode","VILLAGE")}init(){console.log("GameModeSystem INIT"),console.log(`[SYSTEM] GameModeSystem - Strategy Layer ONLINE. Initial Mode: ${this.currentMode}`),c.subscribe("GAME:CHANGE_MODE",e=>{console.log("MODE CHANGE EVENT RECEIVED",e),e.data&&e.data.mode&&this.handleModeChange(e.data.mode)})}handleModeChange(e){if(console.log("MODE CHANGED:",e),e!=="VILLAGE"&&e!=="WORLD_MAP"){console.error(`[GAMEMODE_SYSTEM] Invalid mode requested: ${e}`);return}this.currentMode!==e&&(console.log(`[GAMEMODE_SYSTEM] Perspective shift: ${this.currentMode} -> ${e}`),this.currentMode=e,L.setMode(e))}getCurrentMode(){return this.currentMode}preUpdate(e){}update(e){}postUpdate(e){}destroy(){console.log("[SYSTEM] GameModeSystem - Strategy Layer OFFLINE.")}}const V=new Y;class ${constructor(){r(this,"accumulator",0);r(this,"TICK_INTERVAL",1)}init(){console.log("[SYSTEM] TimeSystem - Global Clock ONLINE.")}preUpdate(e){}update(e){this.accumulator+=e,this.accumulator>=this.TICK_INTERVAL&&(this.emitTick(),this.accumulator-=this.TICK_INTERVAL)}emitTick(){c.emit({type:"GAME:TICK",timestamp:Date.now(),data:{deltaTime:this.TICK_INTERVAL}})}postUpdate(e){}destroy(){console.log("[SYSTEM] TimeSystem - Global Clock OFFLINE.")}}const W=new $;class z{constructor(e,t,o="empty"){r(this,"type","Tile");this.x=e,this.y=t,this.tileType=o}}class H{constructor(){r(this,"WORLD_SIZE",1e3);r(this,"chunkLoaded",new Set)}init(){console.log(`[SYSTEM] WorldSystem - Global Grid Initialized (${this.WORLD_SIZE}x${this.WORLD_SIZE}).`),this.generateEssentialSectors()}generateEssentialSectors(){this.createTile(500,500,"village"),this.createTile(505,505,"resource")}createTile(e,t,o){const n=`${e}:${t}`;if(this.chunkLoaded.has(n))return;const s=i.createEntity();i.addComponent(s,new z(e,t,o)),this.chunkLoaded.add(n)}preUpdate(e){}update(e){}postUpdate(e){}destroy(){console.log("[SYSTEM] WorldSystem - Geography Halted.")}}const K=new H,F={mina_suprimentos:{resource:"wood",baseProduction:10},refinaria:{resource:"iron",baseProduction:5},fabrica_municoes:{resource:"stone",baseProduction:5}};class q{init(){console.log("[SYSTEM] ResourceSystem - Economic Core ONLINE."),c.subscribe("GAME:TICK",e=>{this.generateResources()})}generateResources(){const e=i.getEntitiesWith(["Resource","Building"]);for(const t of e){const o=i.getComponent(t,"Resource"),n=i.getComponent(t,"Building");if(o&&n){const s=F[n.type];if(!s)continue;const a=n.level||1,l=s.baseProduction*a;s.resource==="all"?(o.wood+=l,o.stone+=l,o.iron+=l):o[s.resource]+=l}}}preUpdate(e){}update(e){}postUpdate(e){}destroy(){console.log("[SYSTEM] ResourceSystem - Economic Core OFFLINE.")}}const Q=new q;class Z{constructor(e=[]){r(this,"type","BuildQueue");this.queue=e}}class X{init(){console.log("[SYSTEM] BuildQueueSystem - Logistics Core ONLINE."),c.subscribe(p.BUILDING_UPGRADE_REQUEST,e=>{this.handleUpgradeRequest(e)})}handleUpgradeRequest(e){const t=e.data.id;if(t===void 0)return;const o=i.getComponent(t,"Building");if(!o)return;const n=o.villageId,s=i.getComponent(n,"Resource"),a=i.getComponent(n,"BuildQueue");if(!s||!a){console.error(`[BUILD_SYSTEM] Owner ${n} missing Resource or BuildQueue components.`);return}const l=o.level*150;if(s.wood>=l&&s.stone>=l&&s.iron>=l){s.wood-=l,s.stone-=l,s.iron-=l;const g=o.level*5;a.queue.push({type:"UPGRADE",buildingType:o.buildingType,targetEntityId:t,totalTime:g,remainingTime:g}),console.log(`[BUILD_SYSTEM] Upgrade of ${o.name} (LVL ${o.level} -> ${o.level+1}) initiated for Player ${n}.`),c.emit({type:p.BUILDING_REQUEST,entityId:n,timestamp:Date.now(),data:{status:"STARTED",buildingId:t}})}else console.warn(`[BUILD_SYSTEM] Insufficient resources for upgrade of ${o.name}. Required: ${l}`)}update(e){const t=i.getEntitiesWith(["BuildQueue"]);for(const o of t){const n=i.getComponent(o,"BuildQueue");if(n&&n.queue.length>0){const s=n.queue[0];s.remainingTime-=e,s.remainingTime<=0&&(n.queue.shift(),this.completeTask(s))}}}completeTask(e){if(e.type==="UPGRADE"&&e.targetEntityId!==void 0){const t=i.getComponent(e.targetEntityId,"Building");t&&(t.level+=1,console.log(`[BUILD_SYSTEM] Building ${t.name} upgraded to Level ${t.level}!`),c.emit({type:p.BUILDING_COMPLETED,entityId:e.targetEntityId,timestamp:Date.now(),data:{buildingType:e.buildingType,newLevel:t.level}}))}}preUpdate(e){}postUpdate(e){}destroy(){console.log("[SYSTEM] BuildQueueSystem - Logistics Core OFFLINE.")}}const j=new X;class N{constructor(e=100,t=100){r(this,"type","Health");this.value=e,this.max=t}}class C{constructor(e){r(this,"type","Sprite");this.imagePath=e}}class R{constructor(e=10,t=50,o=1,n=0){r(this,"type","Attack");this.power=e,this.range=t,this.cooldown=o,this.lastAttack=n}}class J{constructor(e="AGGRESSIVE"){r(this,"type","AI");this.behavior=e}}class D{constructor(e,t){r(this,"type","Target");this.x=e,this.y=t}}class ee{constructor(e=!0){r(this,"type","Selection");this.isSelected=e}}class te{init(){console.log("[SYSTEM] OrderSystem - Tactical Downlink ACTIVE."),window.addEventListener("mousedown",this.handleMouseClick.bind(this)),window.addEventListener("contextmenu",e=>e.preventDefault())}preUpdate(e){}update(e){}handleMouseClick(e){const t=e.clientX,o=e.clientY;e.button===0?this.processSelection(t,o):e.button===2&&this.processMoveOrder(t,o)}processSelection(e,t){const o=i.getEntitiesWith(["Position","Sprite"]);let n=!1;i.getEntitiesWith(["Selection"]).forEach(a=>i.removeComponent(a,"Selection"));for(const a of o){const l=i.getComponent(a,"Position");if(Math.sqrt(Math.pow(e-l.x,2)+Math.pow(t-l.y,2))<40){i.addComponent(a,new ee),c.emit({type:p.PLAYER_UNIT_SELECTED,entityId:a,timestamp:Date.now(),data:{x:l.x,y:l.y}}),n=!0;break}}n||c.emit({type:p.PLAYER_SELECTION_CLEARED,timestamp:Date.now(),data:{}})}processMoveOrder(e,t){i.getEntitiesWith(["Selection","Position"]).forEach(n=>{i.addComponent(n,new D(e,t)),c.emit({type:p.PLAYER_MOVE_ORDER,entityId:n,timestamp:Date.now(),data:{targetX:e,targetY:t}})})}postUpdate(e){}destroy(){window.removeEventListener("mousedown",this.handleMouseClick),console.log("[SYSTEM] OrderSystem - Tactical Downlink Offline.")}}const oe=new te;class ie{init(){console.log("[SYSTEM] AISystem - Decision Engine ACTIVE.")}preUpdate(e){}update(e){const t=i.getEntitiesWith(["AI","Position"]),o=i.getEntitiesWith(["Health","Position"]);for(const n of t){if(i.getComponent(n,"Target"))continue;const s=i.getComponent(n,"AI"),a=i.getComponent(n,"Position");s.behavior==="AGGRESSIVE"&&this.pursueTarget(n,a,o)}}pursueTarget(e,t,o){let n=1/0,s=null;for(const a of o){if(a===e)continue;const l=i.getComponent(a,"Position"),m=Math.sqrt(Math.pow(l.x-t.x,2)+Math.pow(l.y-t.y,2));m<n&&(n=m,s=l)}s&&n>20&&i.addComponent(e,new D(s.x,s.y))}postUpdate(e){}destroy(){console.log("[SYSTEM] AISystem - Decision Engine SUSPENDED.")}}const ne=new ie;class se{constructor(){r(this,"speed",2)}init(){console.log("[SYSTEM] MovementSystem - Tactical Navigation ONLINE."),c.subscribe("UNIT:MOVE",e=>{console.log("MOVEMENT ORDER RECEIVED:",e.data),this.handleMoveOrder(e.data)})}handleMoveOrder(e){const{targetX:t,targetY:o}=e;i.getEntitiesWith(["Selection","GridPosition","Velocity"]).forEach(s=>{const a=i.getComponent(s,"Velocity");a&&(a.targetX=t,a.targetY=o,a.isMoving=!0,console.log(`Unit ${s} marching to sector ${t}:${o}`))})}preUpdate(e){}update(e){const t=i.getEntitiesWith(["GridPosition","Velocity"]);for(const o of t){const n=i.getComponent(o,"GridPosition"),s=i.getComponent(o,"Velocity");if(n&&s&&s.isMoving){const a=s.targetX-n.x,l=s.targetY-n.y,m=Math.sqrt(a*a+l*l);if(m<.05)n.x=s.targetX,n.y=s.targetY,s.isMoving=!1,s.vx=0,s.vy=0,console.log(`Unit ${o} reached objective.`);else{const g=a/m*this.speed,E=l/m*this.speed;n.x+=g*e,n.y+=E*e,s.vx=g,s.vy=E}}}}postUpdate(e){}destroy(){console.log("[SYSTEM] MovementSystem - Tactical Navigation OFFLINE.")}}const ae=new se;class de{init(){console.log("[SYSTEM] CombatSystem - Engagement Protocols Normalizing.")}preUpdate(e){}update(e){const t=Date.now()/1e3,o=i.getEntitiesWith(["Attack","Position"]),n=i.getEntitiesWith(["Health","Position"]);for(const s of o){const a=i.getComponent(s,"Attack"),l=i.getComponent(s,"Position");if(!(t-a.lastAttack<a.cooldown))for(const m of n){if(s===m)continue;const g=i.getComponent(m,"Position");if(Math.sqrt(Math.pow(g.x-l.x,2)+Math.pow(g.y-l.y,2))<=a.range){this.executeAttack(s,m,a.power),a.lastAttack=t;break}}}}executeAttack(e,t,o){const n=i.getComponent(t,"Health");n&&(n.value-=o,c.emit({type:p.COMBAT_UNIT_DAMAGED,entityId:t,timestamp:Date.now(),data:{attackerId:e,damage:o,newHealth:n.value}}),n.value<=0&&c.emit({type:p.COMBAT_UNIT_DESTROYED,entityId:t,timestamp:Date.now(),data:{attackerId:e}}))}postUpdate(e){}destroy(){console.log("[SYSTEM] CombatSystem - Engagement Suspended.")}}const le=new de;class re{init(){console.log("[SYSTEM] AttackSystem - Frontline Deployment Operational."),c.subscribe("GAME:TICK",()=>{this.processMarches()})}processMarches(){const e=i.getEntitiesWith(["AttackMarch"]);for(const t of e){const o=i.getComponent(t,"AttackMarch");o&&(o.remainingTime-=1,o.remainingTime<=0&&this.handleMarchArrival(t,o))}}handleMarchArrival(e,t){t.state==="GOING"?(console.log(`[WAR] Attack arrived at [${t.targetX}:${t.targetY}]`),this.resolveCombat(e,t)):t.state==="RETURNING"&&(console.log(`[WAR] Attack returned to origin ${t.originId}`),this.concludeMarch(e,t))}resolveCombat(e,t){const o=i.getEntitiesWith(["Position","Resource"]);let n=null;for(const s of o){const a=i.getComponent(s,"Position");if(Math.round(a.x)===t.targetX&&Math.round(a.y)===t.targetY){n=s;break}}if(n!==null){const s=i.getComponent(n,"Resource");t.loot.wood=Math.floor(s.wood*.5),t.loot.stone=Math.floor(s.stone*.5),t.loot.iron=Math.floor(s.iron*.5),s.wood-=t.loot.wood,s.stone-=t.loot.stone,s.iron-=t.loot.iron,c.emit({type:p.ATTACK_ARRIVED,entityId:e,timestamp:Date.now(),data:{result:"VICTORY",loot:t.loot,targetId:n}})}else c.emit({type:p.ATTACK_ARRIVED,entityId:e,timestamp:Date.now(),data:{result:"EMPTY_TARGET",loot:t.loot}});t.state="RETURNING",t.remainingTime=t.totalTime}concludeMarch(e,t){const o=i.getComponent(t.originId,"Resource");o&&(o.wood+=t.loot.wood,o.stone+=t.loot.stone,o.iron+=t.loot.iron),c.emit({type:p.ATTACK_RETURNED,entityId:e,timestamp:Date.now(),data:{originId:t.originId,finalLoot:t.loot}}),i.removeEntity(e)}preUpdate(e){}update(e){}postUpdate(e){}destroy(){console.log("[SYSTEM] AttackSystem - Frontline Deployment Offline.")}}const ce=new re;class pe{constructor(){r(this,"canvas",null);r(this,"ctx",null);r(this,"images",new Map);r(this,"frameCount",0);r(this,"debug",!0);r(this,"TILE_SIZE",64)}init(){console.log("[SYSTEM] RenderSystem - Initializing Visual Canvas..."),this.createCanvas()}preUpdate(e){}update(e){if(!this.ctx||!this.canvas)return;this.ctx.clearRect(0,0,this.canvas.width,this.canvas.height),this.frameCount++;const t=i.getEntitiesWith(["GridPosition"]);this.debug&&this.frameCount%60===0&&console.log("Entities Rendered (Grid):",t.length),t.forEach(o=>{const n=i.getComponent(o,"GridPosition"),s={x:n.x*this.TILE_SIZE+this.TILE_SIZE/2,y:n.y*this.TILE_SIZE+this.TILE_SIZE/2},a=i.getComponent(o,"Sprite"),l=i.getComponent(o,"Health"),m=i.getComponent(o,"Selection");this.drawEntity(s,a,l,m,o)})}createCanvas(){}drawEntity(e,t,o,n,s){if(!this.ctx)return;const a=32,l=i.getComponent(s,"Player");if(i.getComponent(s,"AI"),n&&(this.ctx.strokeStyle="#00ff00",this.ctx.lineWidth=2,this.ctx.strokeRect(e.x-a/2-2,e.y-a/2-2,a+4,a+4)),o){const E=o.value/o.max;this.ctx.fillStyle="#333333",this.ctx.fillRect(e.x-32/2,e.y-a/2-10,32,4),this.ctx.fillStyle=l?"#22c55e":"#ef4444",this.ctx.fillRect(e.x-32/2,e.y-a/2-10,32*E,4)}}postUpdate(e){}destroy(){this.canvas&&this.canvas.parentNode&&this.canvas.parentNode.removeChild(this.canvas)}}const me=new pe;class ue{constructor(e,t,o,n,s,a,l="GOING",m={wood:0,stone:0,iron:0}){r(this,"type","AttackMarch");this.originId=e,this.targetX=t,this.targetY=o,this.troops=n,this.totalTime=s,this.remainingTime=a,this.state=l,this.loot=m}}class ge{init(){console.log("[SYSTEM] SyncSystem - Uplink Established."),c.subscribe(p.LARAVEL_SYNC_ATTACKS,e=>{this.syncLaravelAttacks(e.data.attacks)})}syncLaravelAttacks(e){e.forEach(t=>{const o=1e4+t.id;if(!i.getEntitiesWith(["AttackMarch"]).includes(o)){const n=Date.now(),s=new Date(t.chegada_em).getTime(),a=Math.round((s-new Date(t.created_at).getTime())/1e3),l=Math.round((s-n)/1e3);l>0&&(i.createEntity(o),i.addComponent(o,new ue(t.origem_base_id,t.destino_x||0,t.destino_y||0,t.tropas||{},a,l,"GOING")))}})}preUpdate(e){}update(e){}postUpdate(e){L.snap()}destroy(){console.log("[SYSTEM] SyncSystem - Uplink Terminated.")}}const he=new ge,f=[B,V,W,K,Q,j,oe,ne,ae,le,ce,me,he];class ye{constructor(){r(this,"lastTime",0);r(this,"running",!1);r(this,"frameId",null)}start(){this.running||(this.running=!0,w.info("[ENGINE] GameLoop starting sequence..."),f.forEach(e=>{try{e.init()}catch(t){console.error("[CRITICAL_FAILURE] System init error:",e,t)}}),this.lastTime=performance.now(),this.frameId=requestAnimationFrame(this.loop.bind(this)))}loop(e){if(!this.running)return;const t=e-this.lastTime;if(t<16){this.frameId=requestAnimationFrame(this.loop.bind(this));return}const o=t/1e3;this.lastTime=e;try{f.forEach(n=>n.preUpdate(o)),f.forEach(n=>n.update(o)),f.forEach(n=>n.postUpdate(o))}catch(n){console.error("[GAMELOOP_EXCEPTION] State integrity compromised:",n)}this.frameId=requestAnimationFrame(this.loop.bind(this))}stop(){this.running=!1,this.frameId!==null&&cancelAnimationFrame(this.frameId),f.forEach(e=>e.destroy()),w.info("[ENGINE] GameLoop halted.")}}const Ee=new ye;class fe{show(){const e=document.getElementById("tactical-hud");e&&(e.style.display="block")}hide(){const e=document.getElementById("tactical-hud");e&&(e.style.display="none")}initialize(){this.createDOM(),this.subscribeToEvents(),console.log("[UI_HUD] Tactical Monitors & Navigation ONLINE.")}createDOM(){var t,o;const e=document.createElement("div");e.id="tactical-hud",e.style.cssText=`
        ...
        `;e.forEach(n=>{const s=i.getComponent(n,"Building");if(s){const a=s.position.x*this.CELL_SIZE,l=s.position.y*this.CELL_SIZE,m=this.selectedBuildingId===n;t+=`
                ...
                `)}this.container.innerHTML=t+o}}const T=new Te;class Se{constructor(){r(this,"container",null)}initialize(){this.createDOM(),console.log("[UI_MAP] World Map View INITIALIZED.")}createDOM(){this.container=document.createElement("div"),this.container.id="world-map-view",this.container.style.cssText=`
            position: absolute;
            top: 150px;
            left: 20px;
            z-index: 1000;
        ...
        `,document.body.appendChild(o)}createScreen(e,t){const o=document.createElement("div");return o.id=t,o.style.position="fixed",o.style.top="0",o.style.left="0",o.style.width="100vw",o.style.height="100vh",o.style.display="none",o.style.flexDirection="column",o.style.alignItems="center",o.style.justifyContent="center",o.style.backgroundColor="transparent",o.style.color="#fff",o.style.fontFamily="monospace",o.style.zIndex="400",o.style.pointerEvents="none",this.screens.set(e,o),o}}const be=new Ie;class O{constructor(e=0,t=0){r(this,"type","Position");this.x=e,this.y=t}}class v{constructor(e=0,t=0){r(this,"type","Velocity");this.vx=e,this.vy=t}}class G{constructor(){r(this,"type","Player")}}class Ae{constructor(e=0,t=0,o=0){r(this,"type","Resource");this.wood=e,this.stone=t,this.iron=o}}class Ce{constructor(e,t=1,o={wood:0,stone:0,iron:0},n="Base_Outpost"){r(this,"type","Village");this.ownerId=e,this.level=t,this.resources=o,this.name=n}}class _{constructor(e,t,o,n,s){r(this,"type","Building");this.name=e,this.buildingType=t,this.level=o,this.position=n,this.villageId=s}}console.log("APP START");console.log("BEFORE GAMELOOP");Ee.start();be.initialize();const u=i.createEntity();i.addComponent(u,new G);const M=i.createEntity();i.addComponent(M,new Ce(u,1,{wood:5e3,stone:5e3,iron:5e3},"Vila Alfa"));i.addComponent(M,{type:"GridPosition",x:5,y:5});i.addComponent(M,{type:"Renderable",renderType:"building"});i.addComponent(u,new Ae(5e3,5e3,5e3));i.addComponent(u,new Z);i.addComponent(u,new v(0,0));i.addComponent(u,new N(1e3,1e3));i.addComponent(u,new R(50,150,1));i.addComponent(u,new C("/images/unidades/blindado_apc.png"));i.addComponent(u,{type:"GridPosition",x:6,y:5});i.addComponent(u,{type:"Renderable",renderType:"unit"});const ve=i.createEntity();i.addComponent(ve,new _("Quartel General","HQ",1,{x:0,y:0},u));const Me=i.createEntity();i.addComponent(Me,new _("Mina de Ferro","MINE",1,{x:1,y:0},u));console.log(`[BOOT] Player Unit Alpha (ID: ${u}) deployed with Spatial Village infrastructure.`);const S=i.createEntity();i.addComponent(S,new G);i.addComponent(S,new O(0,0));i.addComponent(S,new v(0,0));i.addComponent(S,new C("/images/unidades/agente_espiao.png"));console.log(`[BOOT] Test Unit Alpha-Zero (ID: ${S}) deployed at ORIGIN.`);const h=i.createEntity();i.addComponent(h,new O(500,200));i.addComponent(h,new v(0,0));i.addComponent(h,new N(2e3,2e3));i.addComponent(h,new R(100,200,3));i.addComponent(h,new J("AGGRESSIVE"));i.addComponent(h,new C("/images/unidades/tanque_combate.png"));console.log(`[BOOT] Enemy Unit Omega (ID: ${h}) detected with Main Battle Tank.`);i.addComponent(h,{type:"GridPosition",x:10,y:10});i.addComponent(h,{type:"Renderable",renderType:"unit"});A.setState(y.PLAYING);console.log("--- OPERATIONS ACTIVE: VISUAL TACTICAL ENGAGEMENT ONGOING ---");
