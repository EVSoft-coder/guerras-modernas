...
import { C as eventBus, E as Events } from "./app-YntI9fcX.js";
import { g as gameStateService, e as entityManager, G as GameState, s as stateManager, a as GameMode } from "./GameStateService-COR0_MXX.js";
class Logger {
  static info(message) {
...
class CombatSystem {
  ...
  update(deltaTime) {
    ...
    for (const armyId of armies) {
      const vel = entityManager.getComponent(armyId, "Velocity");
      const army = entityManager.getComponent(armyId, "Army");
      const pos = entityManager.getComponent(armyId, "GridPosition");
      if (vel && army && pos && !vel.isMoving && vel.path.length === 0) {
        this.resolveEngagement(armyId, army, pos);
      }
    }
  }
  resolveEngagement(armyId, army, pos) {
...
class UnitComponent {
  constructor(unitCategory, attack, defense, speed, capacity, intelRange = 2, attackBonus = 1, speedBonus = 1) {
    __publicField(this, "type", "Unit");
...
class MarchComponent {
  constructor(origin, target, units, startTime, arrivalTime) {
...
class AttackSystem {
  ...
  launchAttack(data) {
    ...
    entityManager.addComponent(armyId, new MarchComponent(
      ...
      troops,
      Date.now(),
      Date.now() + travelTimeSeconds * 1e3
    ));
    const path = Pathfinding.findPath(
      { x: originX, y: originY },
      { x: targetX, y: targetY },
      () => true
      // Caminho livre por agora
    );
    ...
    const unitType = Object.keys(troops).includes("tanque_combate") ? "tank" : isDrone ? "drone" : "infantry";
    entityManager.addComponent(armyId, new UnitComponent(
      ...
      // Capacity
      isDrone ? 10 : 3
      // Drones têm alcance de visão tática superior
    ));
...
//# sourceMappingURL=index-BT9fDqcG.js.map
