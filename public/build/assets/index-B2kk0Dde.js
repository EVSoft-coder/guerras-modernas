...
import { f as eventBus, E as Events, h as Logger, S as Sr, e as axios } from "./app-DpVMLtDv.js";
import { g as gameStateService, e as entityManager, s as stateManager, G as GameState, a as GameMode } from "./GameStateService-Cqm3O-do.js";
...
const unitConfigs = {
  ...
  politico: {
    id: "politico",
    name: "Líder Político",
    cost: { suprimentos: 4e4, municoes: 5e4, combustivel: 5e4, pessoal: 100 },
    time: 3600,
    attack: 1,
    defenseGeneral: 2,
    defenseArmored: 0,
...
class SyncSystem {
  ...
  async handleBuildingUpgrade(data) {
    ...
    Sr.post("/buildings/upgrade", {
      ...
      preserveState: false,
      // Forçar atualização de props
      preserveScroll: true,
      onSuccess: () => {
        eventBus.emit(Events.ACTION_SUCCESS, { data: { type: "UPGRADE" } });
        Sr.reload({ only: ["state", "resources", "buildingQueue", "buildings"] });
      },
      onError: (err) => {
  ...
  async handleUnitTraining(data) {
    ...
    Sr.post("/units/recruit", {
      ...
      onSuccess: () => {
        ...
        Sr.reload({ only: ["state", "resources", "unitQueue", "units"] });
      },
      onError: (err) => {
        const msg = Object.values(err)[0] || "Mobilização Abortada";
        eventBus.emit(Events.UI_ALERT, { data: { message: msg, type: "error" } });
      }
    });
  }
...
//# sourceMappingURL=index-B2kk0Dde.js.map
